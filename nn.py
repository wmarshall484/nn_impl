...
df = pd.read_csv('/home/will/Downloads/BTC-USD.csv')
pv = df[['Close', 'Volume']]
pv = pv.to_numpy()
...
def train():
    ...
    while True:
        ...
        d_Ws = [(np.random.randint(3, size=x) - 1) * LEARNING_RATE for x in W_sizes]
        d_bs = [(np.random.randint(3, size=(1,x[1])) - 1) * LEARNING_RATE for x in W_sizes]
        ...
        if error < prev_error:
            ...
            for w in Ws:
                print(w)
                print()
            print(f'TRAINING_ERROR({j}): {error}')
            print()
